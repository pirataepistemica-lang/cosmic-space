...
export function Sphere({ config, onClick }: SphereProps) {
  return (
    ...
    >
      <div
        className={`relative h-28 w-28 rounded-full bg-gradient-to-br from-indigo-400/70 via-sky-400/80 to-rose-400/80 shadow-2xl shadow-indigo-500/40 ${config.floatAnimation}`}
      >
        {/* Halo externo suave */}
      ...
      <span className="text-xs font-medium uppercase tracking-[0.2em] text-slate-300/90">
        {config.label}
      </span>
    </motion.button>
  );
...
export function FocusSphere({ config, children, onBack }: FocusSphereProps) {
  return (
    <div
      className="fixed inset-0 z-20 flex items-center justify-center bg-black/70 backdrop-blur-xl"
      aria-modal="true"
      role="dialog"
      aria-label={config.label}
    >
      ...
      <motion.div
        className="relative flex w-full max-w-5xl flex-col gap-6 px-4 py-8 md:px-8"
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={onBack}
            className="inline-flex items-center rounded-full border border-slate-600/70 bg-black/40 px-3 py-1.5 text-xs font-medium text-slate-100 shadow-sm shadow-black/40 transition hover:border-indigo-400 hover:bg-indigo-500/20"
          >
            ← Voltar ao universo
          </button>
        </div>
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-stretch">
          <motion.div
            ...
            layoutId={config.id}
          >
            <div className="relative h-40 w-40 rounded-full bg-gradient-to-br from-indigo-400 via-sky-400 to-rose-500 shadow-2xl shadow-indigo-500/60">
              <div className="absolute inset-[18%] rounded-full bg-black/70 backdrop-blur" />
              <div
          ...
          <div className="md:w-2/3">
            <div className="overflow-hidden rounded-3xl border border-slate-700/70 bg-black/60 p-4 shadow-xl shadow-black/60">
              {children}
            </div>
          </div>
        </div>
